// Day14RegolithReservoir2-optimized.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	std::cout << "start\n";
	while (true) {
		auto sandposition = [&allrockpaths, Sourceofsand, maxy]() {

			std::pair<int, int> sand = Sourceofsand;

			auto it = allrockpaths.find(std::pair<int, int>(sand.first, sand.second));
			if (it != std::end(allrockpaths)) {
				return sand;
			}
		...
		allrockpaths.insert(sandposition);

		sandcount++;

		if (Sourceofsand == sandposition)
			break;
