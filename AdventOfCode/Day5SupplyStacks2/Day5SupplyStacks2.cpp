// Day5SupplyStacks2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
	...
	do
	{
		if (line.substr(0, 4).compare("move") == 0) {
			...
			while (move--) {
 				stacksofcrates[to - 1].push_back(stacksofcrates[from - 1].rbegin()[move]);
				stacksofcrates[from - 1].erase(stacksofcrates[from - 1].end() - move - 1);
			}
