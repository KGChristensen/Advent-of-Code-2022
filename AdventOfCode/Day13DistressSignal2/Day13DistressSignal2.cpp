// Day13DistressSignal2.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
struct packet {
	...
	// For being able to sort 
	friend bool operator==(packet const& left, packet const& right) = default;
};
...
int main()
{
	...
	{
		if (!line.empty())
			allpackets.emplace_back(parsePacket(line));

	}

	//Include two additional divider packets
	packet dividerpacket1 = parsePacket("[[2]]");
	packet dividerpacket2 = parsePacket("[[6]]");
	...
	auto dividerpacket1_location = std::ranges::find(allpackets, dividerpacket1);
	auto dividerpacket2_location = std::ranges::find(allpackets, dividerpacket2);

	std::cout << "The decoder key for the distress signal: " << ((std::ranges::distance(allpackets.begin(), dividerpacket1_location) + 1) * (std::ranges::distance(allpackets.begin(), dividerpacket2_location) + 1)) << "\n";
